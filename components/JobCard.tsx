...
export default function JobCard({ job }: { job: PublicJob }) {
  ...
  return (
    <div className="pinned-card block p-5 pl-6 hover:border-mustard transition-colors">
      <a
        href={job.url}
        target="_blank"
      ...
      >
        {job.logo ? (
          ...
          <img src={job.logo} alt="" className="h-10 w-10 object-contain shrink-0" />
        ) : (
        ...
        <div className="min-w-0 flex-1">
          <h3 className="font-display text-lg leading-snug text-ink truncate">{job.title}</h3>
          ...
          <div className="mt-2 flex flex-wrap gap-x-3 gap-y-1 text-sm text-ink/60">
            ...
            {salary && <span className="text-denim font-medium">{salary}</span>}
          </div>

          <p className="mt-2 text-xs text-ink/50">{timeAgo(job.posted_date)}</p>
        </div>
      </a>

      <Link
        href={matchHref}
        className="mt-3 inline-block text-xs font-medium text-denim hover:underline"
      >
        Match my resume →
      </Link>
    </div>
