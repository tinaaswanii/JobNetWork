...
import { calculateResumeMatchWithAI } from "@/lib/llm-match";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const jobInput = {
      ...
      exp_min: typeof job.exp_min === "number" ? job.exp_min : null,
      exp_max: typeof job.exp_max === "number" ? job.exp_max : null,
    };

    const aiResult = await calculateResumeMatchWithAI(text, jobInput);
    const result: ResumeMatchResult = aiResult ?? calculateResumeMatch(text, jobInput);
